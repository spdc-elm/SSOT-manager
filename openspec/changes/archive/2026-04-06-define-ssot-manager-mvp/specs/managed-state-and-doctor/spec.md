...
The system SHALL store a durable last-successful-apply journal that captures the filesystem mutations needed to undo the apply.
...
The system SHALL compare managed records against the live filesystem and report at least broken symlinks, missing managed targets, and targets whose current state no longer matches the recorded managed expectation.
...
#### Scenario: Manual drift is reported
- **WHEN** a managed target has been manually replaced or edited after apply
...
The system MUST limit `undo` to the most recent successful apply journal and MUST refuse to modify targets that are not covered by that recorded journal.
